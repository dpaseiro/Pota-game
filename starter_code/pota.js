  ...
  setInterval(function() {
    ...
    for (var i = 0; i < that.creeps.length; i++) {
      ...
      if (
        ...
        that.sven.x + that.sven.width >= that.creeps[i].x &&
...
Sven.prototype.rockBottom = function() {
  console.log(this.y);

  if (this.y >= 720) {
    game.stop();
    // console.log(gameCtx.gameOver);
    if(gameCtx.gameOver === false){
  ...
  if (this.canAttack) {
    ...
    for (var i = 0; i < game.creeps.length; i++) {
      ...
      if (
        ...
        game.creeps[i].x + game.creeps[i].width >= game.sword.x + 88 - 43 &&
